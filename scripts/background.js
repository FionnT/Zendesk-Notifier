...
function run(){
  ...
  chrome.storage.sync.get(['queue_count'], function(result) {
    ...
    if(count>=1) {
      ...
      chrome.browserAction.setIcon({path: {16: "icons/icon16.png", 48: "icons/icon48.png", 128: "icons/icon128.png"}});
...
chrome.tabs.onRemoved.addListener(function(tabid, removed) {
  ...
  chrome.storage.sync.get(['enable'], function(result){
    if(result.enable === true){
      chrome.storage.sync.get(["livetab"], function(result){
        if((result.livetab != null && result.livetab != undefined) && result.livetab === curtab){
          chrome.alarms.clearAll();
          chrome.storage.sync.set({livetab: null});
          chrome.storage.sync.set({enable: false});
          var today = new Date();
          ...
          chrome.browserAction.setIcon({path: {16: "icons/icon16d.png", 48: "icons/icon48d.png", 128: "icons/icon128d.png"}})
          run()
        }
      });
    }else {
...
chrome.runtime.onMessage.addListener( function(request, sender, sendResponse) {
  ...
  if(request.notification != undefined && request.notification != null){
